...
def cmd_get_workflow_guide(use_case: str) -> str:
    ...
    if use_case == "close_position":
        return (
            ...
            "Choose the path by intent (FIFO netting vs ForceOpen / explicit leg close).\n\n"
            "A. FIFO / netting close (position NOT opened with IsForceOpen=True)\n"
            "Place a standard offsetting order (opposite BuySell, same Uic/Amount).\n"
            '- OrderRelation: "StandAlone" (typical standalone order)\n'
            "- No PositionId required; oldest open legs net first.\n"
            "- Prefer library: PositionClose.fifo_market / fifo_limit / fifo_stop "
            "(or SaxoClient.close_fifo_*).\n\n"
            "B. ForceOpen / hedge residue / explicit position close (CRITICAL)\n"
            "A standalone opposite Market/Limit/Stop does NOT close an IsForceOpen leg — "
            "it opens a new opposite position (fake flatten).\n"
            "Correct body shape (SIM-verified):\n"
            "- Top-level PositionId (from PositionsQuery item, not only PositionBase)\n"
            "- Nested Orders: [ { Uic, AssetType, Amount, BuySell, OrderType, ... } ]\n"
            "- Prefer library: PositionClose.force_open_market / force_open_limit / "
            "force_open_stop (or SaxoClient.close_force_open_*).\n"
            "- Cleanup residue: ClearForceOpen=True market "
            "(PositionClose.clear_force_open_market / SaxoClient.flatten_force_open).\n\n"
            "Note on OrderRelation=ClosePosition:\n"
            "Some docs mention OrderRelation=ClosePosition with PositionId. "
            "Do not rely on that alone for ForceOpen — use the nested Orders[] shape. "
            "OrderRelation=ClosePosition without the nested body may error or no-op.\n\n"
            "CRITICAL PITFALL: EndOfDayNetting (EOD) vs Intraday\n"
            "Some demo accounts default to EndOfDay netting.\n"
            "- EndOfDay: Closed positions may STILL APPEAR during the day (zombie positions).\n"
            "- Intraday: Closed positions disappear in real-time.\n"
            "Also read saxo://docs/pitfalls.md section 1b (ForceOpen close)."
        )
